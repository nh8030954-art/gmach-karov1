...
async function orgLifecycle(request,env,id){
  ...
  if(a==="close"){
    ...
    const affected=await qall(env,`SELECT lr.id,lr.borrower_id,lr.status,i.title FROM loan_requests lr JOIN items i ON i.id=lr.item_id WHERE i.organization_id=? AND lr.status IN ('pending','approved')`,[id]);
    for(const row of affected){
      if(b.cancelPending===true) await qrun(env,"UPDATE loan_requests SET status='cancelled',workflow_status='cancelled',cancelled_by=?,cancelled_at=?,updated_at=? WHERE id=?",[user.id,now,now,row.id]);
      await env.DB.prepare("INSERT INTO notifications(id,user_id,type,title,body,request_id) VALUES(?,?,'status',?,?,?)").bind(crypto.randomUUID(),row.borrower_id,b.cancelPending===true?"הבקשה בוטלה עקב סגירה זמנית":"הגמ״ח נסגר זמנית",reopens?`מועד פתיחה צפוי: ${reopens}`:"נעדכן כשהגמ״ח ייפתח מחדש.",row.id).run();
    }
  }
...
const orgs=await qall(env,"SELECT id,deletion_requested_at FROM organizations WHERE deletion_requested_at IS NOT NULL AND deleted_at IS NULL LIMIT 100",[]);for(const o of orgs){if(Date.now()-Date.parse(o.deletion_requested_at)<7*86400000)continue;if(await qfirst(env,"SELECT 1 FROM loan_requests lr JOIN items i ON i.id=lr.item_id WHERE i.organization_id=? AND lr.status IN ('pending','approved','collected') LIMIT 1",[o.id]))continue;const h=await qfirst(env,"SELECT 1 FROM loan_requests lr JOIN items i ON i.id=lr.item_id WHERE i.organization_id=? LIMIT 1",[o.id]);if(h){await env.DB.batch([env.DB.prepare("DELETE FROM organization_invitations WHERE organization_id=?").bind(o.id),env.DB.prepare("DELETE FROM organization_members WHERE organization_id=? AND role!='owner'").bind(o.id),env.DB.prepare("UPDATE organizations SET deleted_at=?,is_hidden=1,name='גמ״ח שנמחק',description='הגמ״ח אינו פעיל עוד',phone=NULL,address=NULL,website_url=NULL WHERE id=?").bind(now,o.id)]);}else await qrun(env,"DELETE FROM organizations WHERE id=?",[o.id]);}}
