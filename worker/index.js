...
async function createRequestMessage(request, env, requestId) {
  ...
  if (row.status === "returned" && row.returned_at && Date.now() - Date.parse(row.returned_at) > 14 * 86400000) {
    throw new HttpError(410, "הצ׳אט נסגר 14 ימים לאחר החזרת הפריט");
  }
  const recipientId = row.borrower_id === user.id ? row.owner_id : row.borrower_id;
  ...
  if (block && (!block.effective_after_request_id || block.effective_after_request_id !== requestId || row.status === "returned")) {
    throw new HttpError(403, "לא ניתן לשלוח הודעות בין המשתמשים");
  }
  const body = await readJson(request);
  ...
  if (links.length > 3 || links.some(link => /(?:bit\.ly|tinyurl\.com|t\.co|cutt\.ly|rb\.gy)/i.test(link))) {
    await env.DB.prepare("INSERT INTO security_events(id,user_id,event_type,severity,details_json) VALUES(?,?,?,?,?)")
      .bind(crypto.randomUUID(),user.id,"suspicious_chat_link","warning",JSON.stringify({requestId,links:links.slice(0,5)})).run();
...
async function listRequestMessages(request, env, requestId) {
  ...
  return json({
    request: { id: row.id, status: row.status, itemTitle: row.item_title, organizationName: row.org_name, chatClosed: Boolean(closed) },
    messages: result.results.map(message => ({ ...message, isMine: message.sender_id === user.id }))
  });
}
