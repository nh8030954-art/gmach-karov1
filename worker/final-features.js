...
export async function handleFinalFeatures(request,env,ctx,url){
  ...
  try{
    ...
    m=path.match(/^\/api\/admin\/holiday-rules\/([^/]+)$/);if(m&&method==="PATCH")return await holidays(request,env,decodeURIComponent(m[1]));
    if(path==="/api/admin/moderation"&&method==="GET")return await moderation(request,env);
    m=path.match(/^\/api\/admin\/moderation\/([^/]+)$/);if(m&&method==="PATCH")return await moderation(request,env,decodeURIComponent(m[1]));
    if(path==="/api/admin/audit"&&method==="GET")return await auditList(request,env,url);
    if(path==="/api/admin/analytics/operations"&&method==="GET")return await opsAnalytics(request,env);
