(() => {
  ...
  $("#request-form").addEventListener("submit", async event => { event.preventDefault(); const form=event.currentTarget; const button=event.submitter; setButtonBusy(button,true,"שולחים בקשה…"); try { const result=await api("/api/loan-requests",{method:"POST",body:{itemId:$("#request-item-id").value,requestedFrom:$("#request-start").value,requestedUntil:$("#request-end").value,quantity:Number($("#request-quantity").value),phone:$("#request-phone").value,note:$("#request-note").value,depositAccepted:$("#request-deposit-consent").checked}}); form.reset(); closeDialog($("#request-dialog")); toast(result.request.status==="approved"?"ההזמנה אושרה ונשמרה.":"הבקשה נשלחה למנהל הגמ״ח. נעדכן אתכם כשיש תשובה."); await loadItems(); } catch(error){toast(error.message||"לא הצלחנו לשלוח את הבקשה","error");} finally{setButtonBusy(button,false);} });
    $("#gmach-form").addEventListener("submit", async event => { event.preventDefault(); const form=event.currentTarget; const button = event.submitter; const editing = state.editingOrganizationId; setButtonBusy(button, true, editing ? "שומרים…" : "שולחים לבדיקה…"); try { const body = { name: $("#gmach-name").value, primaryCategory: $("#gmach-category").value, city: $("#gmach-city").value, neighborhood: $("#gmach-neighborhood").value, description: $("#gmach-description").value, phone: $("#gmach-phone").value, address:$("#gmach-address").value,serviceArea:$("#gmach-service-area").value,hours:{"שעות":$("#gmach-hours").value},pickupOptions:$$('[name="gmach-pickup"]:checked').map(input=>input.value) }; await api(editing ? `/api/organizations/${encodeURIComponent(editing)}` : "/api/organizations", { method: editing ? "PATCH" : "POST", body }); form.reset(); state.editingOrganizationId = null; closeDialog($("#gmach-dialog")); toast(editing ? "פרטי הגמ״ח נשמרו" : "עמוד הגמ״ח נשלח לבדיקה. נעדכן אתכם כשיאושר."); await refreshAccountSnapshot(); if (editing) showDashboard("gmachim"); } catch (error) { toast(error.message || "לא הצלחנו לשמור את הגמ״ח", "error"); } finally { setButtonBusy(button, false); } });
