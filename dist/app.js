(() => {
  ...
  async function loadSiteSettings() {
    ...
    try {
      ...
      const siteName=s.site_name||"גמ״ח ברגע", tagline=s.tagline||"גדולה גמילות חסדים יותר מן הצדקה", logo=!s.logo_url||s.logo_url==="/gmach-berega-mark.jpg"||s.logo_url==="/gmach-berega-logo.jpg"?"/gmach-berega-mark-cropped.jpg":s.logo_url; $$(".brand-copy strong").forEach(el => el.textContent = siteName); $$(".brand-copy small").forEach(el => el.textContent = tagline); $("#hero-title").textContent = s.hero_title||"מה תרצו לשאול היום?"; $("#hero-title").nextElementSibling.textContent = s.hero_description||"מוצאים ציוד להשאלה בחינם מגמ״חים ואנשים טובים קרוב לבית."; $$(".brand-logo-crop img").forEach(img => img.src = logo); document.title = `${siteName} — ${tagline}`;
