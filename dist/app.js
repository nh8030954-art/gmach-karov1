(() => {
  ...
  async function saveVisualElement(event) {
    ...
    const styles = { fontFamily: $("#ve-font").value, fontSize: $("#ve-size").value, color: $("#ve-color").value, backgroundColor: $("#ve-bg").value, textAlign: $("#ve-align").value, width: $("#ve-width").value, marginTop: $("#ve-mt").value, marginBottom: $("#ve-mb").value, paddingTop: $("#ve-padding").value, paddingBottom: $("#ve-padding").value, paddingInlineStart: $("#ve-padding").value, paddingInlineEnd: $("#ve-padding").value, order: $("#ve-order").value, transform: `translate(${$("#ve-x").value || 0}px, ${$("#ve-y").value || 0}px)` };
    try { const data = await api("/api/admin/page-customizations", { method: "PUT", body: { key, text: leaf ? $("#ve-text").value : null, styles, attributes: { href: $("#ve-href").value || undefined, hidden: $("#ve-hidden").checked, disabled: $("#ve-disabled").checked } } }); state.customizations.set(key, data.customization); applyPageCustomization(data.customization); element.classList.add("is-edit-selected"); toast("השינוי נשמר ונוצרה גרסה לשחזור"); } catch (error) { toast(error.message, "error"); }
  }
  ...
  async function init() {
    setupEvents(); setAuthMode("login"); updateAuthUI(); await detectServer(); await loadSiteSettings(); await loadPageCustomizations(); await refreshUser(); await loadItems(); registerWebMCP();
