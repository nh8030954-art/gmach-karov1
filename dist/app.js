(() => {
  ...
  function applyFilters({ resetVisible = true } = {}) {
    ...
    state.filteredItems.sort((a, b) => sort === "newest" ? new Date(b.created_at) - new Date(a.created_at) : sort === "city" ? String(a.city).localeCompare(String(b.city), "he") : Number(b.availability_status === "available") - Number(a.availability_status === "available") || Number(b.rating || 0) - Number(a.rating || 0));
    renderItems();
  }
  ...
  async function openOrganization(id) {
    if (!id) return;
    try { const data = await api(`/api/organizations/${encodeURIComponent(id)}/public`); const org=data.organization, items=data.items||[], reviews=data.reviews||[], hours=Object.entries(org.hours||{}); $("#organization-dialog-content").dataset.organizationId=String(id);
      $("#organization-dialog-content").innerHTML = `<article class="organization-detail"><div class="organization-hero"><span class="verification-chip">${org.rating ? `⭐ ${escapeHTML(org.rating)} מתוך 5` : "חדש — ללא דירוגים"}</span><h2 id="organization-title">${escapeHTML(org.name)}</h2><p>${escapeHTML(org.description)}</p><div class="organization-meta"><span>📍 ${escapeHTML([org.city,org.neighborhood].filter(Boolean).join(", "))}</span><span>${org.rating ? `⭐ ${escapeHTML(org.rating)} (${Number(org.review_count||0)} דירוגים)` : "עדיין אין דירוגים"}</span><span>${org.last_active_at ? `פעיל ${escapeHTML(formatRelative(org.last_active_at))}` : "פעילות טרם עודכנה"}</span></div></div><div class="organization-facts"><section><h3>איך מקבלים?</h3><p>${escapeHTML((org.pickupOptions||[]).map(v => ({pickup:"איסוף עצמי",delivery:"משלוח",coordination:"בתיאום"}[v]||v)).join(" · ") || "בתיאום")}</p></section><section><h3>כתובת ויצירת קשר</h3><p>${escapeHTML(org.address || [org.city,org.neighborhood].filter(Boolean).join(", "))}${org.contact_phone ? ` · <a href="tel:${escapeHTML(org.contact_phone)}" dir="ltr">${escapeHTML(org.contact_phone)}</a>` : ""}</p></section><section><h3>שעות פעילות</h3><p>${hours.length ? hours.map(([d,h]) => `${escapeHTML(d)}: ${escapeHTML(h)}`).join(" · ") : "בתיאום מראש"}</p></section><section><h3>אזור שירות</h3><p>${escapeHTML(org.service_area || org.city)}</p></section></div><h3>פריטים בגמ״ח</h3><div class="mini-items">${items.map(item => `<button type="button" data-org-item="${escapeHTML(item.id)}"><strong>${escapeHTML(item.title)}</strong><span>${item.rating ? `⭐ ${escapeHTML(item.rating)}` : escapeHTML(item.availability_status === "available" ? "זמין" : "בתיאום")}</span></button>`).join("") || "אין כרגע פריטים פעילים"}</div><h3>דירוגי משתמשים</h3><div class="review-list">${reviews.map(review => `<blockquote data-review-id="${escapeHTML(review.id||"")}"><strong>גמ״ח ${"★".repeat(Number(review.rating))}</strong>${review.item_rating ? `<strong>פריט ${"★".repeat(Number(review.item_rating))}</strong>` : ""}${review.service_rating ? `<strong>שירות ${"★".repeat(Number(review.service_rating))}</strong>` : ""}<p>${escapeHTML(review.comment || "ללא הערה")}</p><small>${review.item_title ? escapeHTML(review.item_title) : ""}${review.branch_name ? ` · ${escapeHTML(review.branch_name)}` : ""}${review.returned_at || review.requested_from ? ` · ${escapeHTML((review.returned_at||review.requested_from).slice(0,10))}` : ""}</small><cite>${escapeHTML(review.author_name)}</cite>${review.organization_response ? `<p><strong>תגובת הגמ״ח:</strong> ${escapeHTML(review.organization_response)}</p>` : ""}<div class="review-actions"><button type="button" class="button button-secondary" data-review-helpful="${escapeHTML(review.id||"")}">מועילה (${Number(review.helpful_count||0)})</button><button type="button" class="button button-secondary" data-review-report="${escapeHTML(review.id||"")}">דיווח</button></div></blockquote>`).join("") || "עדיין אין דירוגים"}</div></article>`;
  ...
  function startRequest(id) { const item = findItem(id); if (!item) return; requireAuth(() => { closeDialog($("#item-dialog")); $("#request-form").reset(); $("#request-item-id").value=item.id; $("#request-item-name").textContent=item.title; $("#request-quantity").max=String(item.quantity||1); $("#request-quantity").value="1"; $("#request-availability").textContent="בחרו תאריך ושעה כדי לבדוק זמינות."; $("#request-deposit-box").hidden=true; $("#request-deposit-consent").required=false; openDialog($("#request-dialog")); }); }
  let availabilityTimer;
  async function checkRequestedAvailability(){
    clearTimeout(availabilityTimer);
    const id=$("#request-item-id").value,from=$("#request-start").value,until=$("#request-end").value;
    if(!id||!from||!until){$("#request-availability").textContent="בחרו תאריך ושעה כדי לבדוק זמינות.";return;}
    availabilityTimer=setTimeout(async()=>{try{
      ...
      $("#request-availability").textContent=data.available?`זמין בטווח שבחרתם — ${data.availableQuantity} יחידות זמינות.`:(data.nextAvailableAt?`לא זמין בטווח שבחרתם. הזמינות הקרובה שמצאנו מתחילה ב־${new Date(data.nextAvailableAt).toLocaleString("he-IL")}.`:"לא זמין בטווח שבחרתם."); $("#request-waitlist").hidden=Boolean(data.available);
  ...
  async function openItemForm(itemId = null) {
    requireAuth(async () => { try {
      ...
      if (item) {
        ...
        $("#item-quantity").value = item.quantity || 1; $("#item-description").value = item.description || ""; $("#item-conditions").value = item.loan_conditions || ""; $("#item-type").value=item.item_type||"loan"; $("#item-pickup").value=item.pickup_method||"pickup"; $("#item-subcategory").value=item.subcategory||""; $("#item-tags").value=(item.tags||[]).join(", "); $("#item-min-loan").value=item.min_loan_minutes||60; $("#item-max-loan").value=item.max_loan_minutes||10080; $("#item-booking-notice").value=item.booking_notice_minutes||0; $("#item-turnaround").value=item.turnaround_minutes||0; $("#item-booking-horizon").value=item.booking_horizon_days||365; $("#item-approval-mode").value=item.approval_mode||"manual"; $("#item-deposit-required").checked=Boolean(item.deposit_required); $("#item-deposit-amount-row").hidden=!item.deposit_required; $("#item-deposit-amount").value=(Number(item.deposit_amount_agorot||0)/100).toFixed(2); $("#item-free").checked = true;
      } else { $("#item-quantity").value=1; $("#item-condition").value="מצוין"; $("#item-min-loan").value=60; $("#item-max-loan").value=10080; $("#item-booking-notice").value=0; $("#item-turnaround").value=0; $("#item-booking-horizon").value=365; $("#item-approval-mode").value="manual"; $("#item-deposit-required").checked=false; $("#item-deposit-amount-row").hidden=true; $("#item-deposit-amount").value=0; $("#item-free").checked=true; }
  ...
  function renderDashboardItems(items) {
    ...
    document.querySelectorAll('[data-edit-item]').forEach(button => button.addEventListener("click", () => openItemForm(button.dataset.editItem))); document.querySelectorAll('[data-inventory-item]').forEach(button=>button.addEventListener("click",()=>openInventoryManager(button.dataset.inventoryItem))); $$('[data-toggle-item]').forEach(button => button.addEventListener("click", () => toggleItemAvailability(button.dataset.toggleItem, button.dataset.currentAvailability))); $$('[data-archive-item]').forEach(button => button.addEventListener("click", () => archiveItem(button.dataset.archiveItem, button.dataset.isArchived !== "true")));
  }
  ...
  function setupEvents() {
    ...
    $("#auth-form").addEventListener("submit", async event => {
      ...
      try { if (!state.serverAvailable) throw new Error("ההרשמה תהיה זמינה לאחר הפרסום ב־Cloudflare"); const body = { email: $("#auth-email").value.trim(), password: $("#auth-password").value }; if (state.authMode === "register") { Object.assign(body,{fullName:$("#auth-name").value.trim(),phone:$("#auth-phone").value.trim(),city:$("#auth-city").value.trim(),address:$("#auth-address").value.trim(),termsAccepted:$("#auth-consent").checked,operationalEmailsAccepted:$("#auth-operational-consent").checked,communityEmailsAccepted:$("#auth-community-consent").checked,turnstileToken:window.GmachTurnstile?.token("register")||null}); } let data = await api(`/api/auth/${state.authMode}`, { method: "POST", body }); if (data.verificationRequired) { state.pendingVerificationEmail = data.email; form.reset(); closeDialog($("#auth-dialog")); $("#verify-email-code").value = ""; openDialog($("#verify-email-dialog")); toast("קוד אימות נשלח למייל"); return; } if (data.requiresTwoFactor) { const code = window.prompt("הקלידו את הקוד מאפליקציית Authenticator"); if (!code) throw new Error("הכניסה בוטלה"); data = await api("/api/auth/2fa/verify-login", { method: "POST", body: { challenge: data.challenge, code } }); } await finishAuthentication(data.user, form); }
  ...
  $("#request-waitlist").addEventListener("click",async()=>{try{await api("/api/items/"+encodeURIComponent($("#request-item-id").value)+"/waitlist",{method:"POST",body:{requestedFrom:$("#request-start").value,requestedUntil:$("#request-end").value,quantity:Number($("#request-quantity").value)}});toast("נוספתם לרשימת ההמתנה.");$("#request-waitlist").hidden=true;}catch(e){toast(e.message||"לא הצלחנו להצטרף לרשימת ההמתנה","error")}});
  $("#request-form").addEventListener("submit", async event => { event.preventDefault(); const form=event.currentTarget; const button=event.submitter; setButtonBusy(button,true,"שולחים בקשה…"); try { const result=await api("/api/loan-requests",{method:"POST",body:{itemId:$("#request-item-id").value,requestedFrom:$("#request-start").value,requestedUntil:$("#request-end").value,quantity:Number($("#request-quantity").value),phone:$("#request-phone").value,note:$("#request-note").value,depositAccepted:$("#request-deposit-consent").checked}}); form.reset(); closeDialog($("#request-dialog")); toast(result.request.status==="approved"?"ההזמנה אושרה ונשמרה.":"הבקשה נשלחה למנהל הגמ״ח. נעדכן אתכם כשיש תשובה."); await loadItems(); } catch(error){toast(error.message||"לא הצלחנו לשלוח את הבקשה","error");} finally{setButtonBusy(button,false);} });
    $("#gmach-form").addEventListener("submit", async event => { event.preventDefault(); const form=event.currentTarget; const button = event.submitter; const editing = state.editingOrganizationId; setButtonBusy(button, true, editing ? "שומרים…" : "פותחים גמ״ח…"); try { const body = { name: $("#gmach-name").value, primaryCategory: $("#gmach-category").value, city: $("#gmach-city").value, neighborhood: $("#gmach-neighborhood").value, description: $("#gmach-description").value, phone: $("#gmach-phone").value, address:$("#gmach-address").value,serviceArea:$("#gmach-service-area").value,hours:{"שעות":$("#gmach-hours").value},pickupOptions:$$('[name="gmach-pickup"]:checked').map(input=>input.value) }; await api(editing ? `/api/organizations/${encodeURIComponent(editing)}` : "/api/organizations", { method: editing ? "PATCH" : "POST", body }); form.reset(); state.editingOrganizationId = null; closeDialog($("#gmach-dialog")); toast(editing ? "פרטי הגמ״ח נשמרו" : "עמוד הגמ״ח נפתח והוא פעיל עכשיו"); await Promise.all([refreshAccountSnapshot(),loadDiscovery()]); if (editing) showDashboard("gmachim"); } catch (error) { toast(error.message || "לא הצלחנו לשמור את הגמ״ח", "error"); } finally { setButtonBusy(button, false); } });
    $("#item-form").addEventListener("submit", async event => { event.preventDefault(); const form=event.currentTarget; const button = event.submitter; const editing = state.editingItemId; setButtonBusy(button, true, editing ? "שומרים…" : "מפרסמים…"); try { const body={organizationId:$("#item-gmach").value,title:$("#item-name").value,category:$("#item-category").value,condition:$("#item-condition").value,quantity:Number($("#item-quantity").value),description:$("#item-description").value,loanConditions:$("#item-conditions").value,itemType:$("#item-type").value,pickupMethod:$("#item-pickup").value,subcategory:$("#item-subcategory").value,tags:$("#item-tags").value.split(","),minLoanMinutes:Number($("#item-min-loan").value),maxLoanMinutes:Number($("#item-max-loan").value),bookingNoticeMinutes:Number($("#item-booking-notice").value),turnaroundMinutes:Number($("#item-turnaround").value),bookingHorizonDays:Number($("#item-booking-horizon").value),approvalMode:$("#item-approval-mode").value,depositRequired:$("#item-deposit-required").checked,depositAmount:Number($("#item-deposit-amount").value||0)}; const data = await api(editing ? `/api/items/${encodeURIComponent(editing)}` : "/api/items", { method: editing ? "PATCH" : "POST", body }); const itemId = editing || data.item.id; await uploadItemImages(itemId, [...$("#item-images").files]); form.reset(); state.editingItemId = null; $("#item-gmach").disabled = false; closeDialog($("#item-form-dialog")); toast(editing ? "השינויים נשמרו ונשלחו לבדיקה לפי הצורך" : "הפריט נשמר ונשלח לבדיקה לפני פרסום"); await refreshAccountSnapshot(); if (editing) showDashboard("items"); } catch (error) { toast(error.message || "לא הצלחנו לשמור את הפריט", "error"); } finally { setButtonBusy(button, false); } });
