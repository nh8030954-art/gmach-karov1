(() => {
  ...
  async function renderAdmin() {
    const [data, overview, appearance, userData] = await Promise.all([api("/api/admin/pending"), api("/api/admin/overview"), api("/api/admin/site-settings"), api("/api/admin/users")]), organizations = data.organizations || [], items = data.items || [], reports = data.reports || [];
    ...
    $("#dashboard-content").innerHTML = `<section class="admin-console"><div class="admin-overview"><article><strong>${overview.stats.users}</strong><span>משתמשים</span></article><article><strong>${overview.stats.organizations}</strong><span>גמ״חים</span></article><article><strong>${overview.stats.items}</strong><span>פריטים</span></article><article><strong>${overview.stats.requests}</strong><span>בקשות</span></article></div>
      ...
      <details><summary>משתמשים (${userData.users.length})</summary><div class="admin-users">${userData.users.map(u => `<article class="dashboard-row"><div><h3>${escapeHTML(u.full_name)}</h3><p dir="ltr">${escapeHTML(u.email)}</p></div><div><span class="status-chip ${u.account_status === "active" ? "active" : "rejected"}">${u.account_status === "active" ? "פעיל" : "מושעה"}</span><p>${u.role === "admin" ? "מנהל" : "משתמש"} · ${u.email_verified ? "מייל מאומת" : "מייל לא מאומת"} · ${u.totp_enabled ? "2FA פעיל" : "ללא 2FA"}</p></div></article>`).join("")}</div></details>
  ...
  async function init() {
    setupEvents(); setAuthMode("login"); updateAuthUI(); await detectServer(); await loadSiteSettings(); await refreshUser(); await loadItems(); registerWebMCP();
