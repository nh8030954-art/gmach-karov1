(() => {
  ...
  const INFO_CONTENT = Object.freeze({
    ...
    terms: '<h2 id="info-dialog-title">תנאי שימוש</h2><p>השימוש בגמ״ח ברגע כפוף לכללים הבאים. השירות משמש פלטפורמת תיווך קהילתית ואינו צד להסכם ההשאלה בין המשתמשים.</p><h3>שירות חינמי בלבד</h3><p>אסור לדרוש תשלום עבור פרסום, בקשה או השאלה. ניתן לקבוע פיקדון סביר רק אם הוא מצוין מראש ואינו הופך לתשלום מוסווה.</p><h3>תוכן וציוד מותרים</h3><p>המפרסם אחראי לתיאור מדויק, לזכותו למסור את הפריט ולתקינות סבירה. אסור לפרסם ציוד בלתי חוקי, מסוכן, מזויף או מסחרי.</p><h3>אחריות המשתמשים</h3><p>השואל אחראי לשימוש זהיר ולהחזרה במועד. יש לבדוק ציוד רגיש לפני שימוש. הפלטפורמה אינה מבטחת את הפריטים ואינה אחראית לנזק שנגרם מהשימוש בהם.</p><h3>אכיפה וסיום שימוש</h3><p>מנהל האתר רשאי להסיר פרסום, לעצור חשבון או לשמור תיעוד הנדרש לטיפול בדיווח ובהפרת כללים.</p>',
    privacy: '<h2 id="info-dialog-title">מדיניות פרטיות</h2><p>נאספים רק הפרטים הדרושים להפעלת השירות: שם, כתובת אימייל, טלפון לתיאום, פרסומים, בקשות, שיחות ודיווחים.</p><h3>שימוש במידע</h3><p>המידע משמש לניהול החשבון, אימות משתמשים, תיאום השאלות, אבטחה, מניעת שימוש לרעה וטיפול בפניות.</p><h3>מי רואה מה?</h3><p>פרטי קשר אינם מוצגים בקטלוג הפתוח. הם זמינים רק לצדדים הרלוונטיים לבקשה ובהתאם להרשאות. מנהל האתר יכול לגשת למידע לצורכי תמיכה ואבטחה.</p><h3>אחסון ואבטחה</h3><p>המידע נשמר ב־Cloudflare D1 והתמונות ב־Cloudflare R2. הסיסמאות נשמרות כגיבוב בלבד. הגישה נאכפת בשרת ומוגנת בעוגיות מאובטחות.</p><h3>מחיקה ושליטה</h3><p>משתמש רגיל יכול למחוק את החשבון והמידע המקושר מתוך האזור האישי. מידע מסוים עשוי להישמר לזמן מוגבל אם הוא נחוץ לטיפול בדיווח, אבטחה או חובה חוקית.</p><h3>ספקים</h3><p>Cloudflare מספקת אחסון ותשתית, ו־Resend משמש לשליחת הודעות אימות לאחר חיבורו. הספקים מקבלים רק את המידע הדרוש לביצוע השירות.</p>'
  });
