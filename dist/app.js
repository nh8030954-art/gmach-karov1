(() => {
  ...
  const STATUS_LABELS = Object.freeze({ pending: "ממתינה לאישור זמן איסוף", awaiting_pickup_approval: "המלאי נשמר — ממתין לאישור זמן איסוף", alternative_time_proposed: "הוצע זמן חלופי", approved: "אושר ומוכן לאיסוף", approved_ready: "אושר ומוכן לאיסוף", declined: "נדחה", cancelled: "בוטל", cancelled_by_borrower: "בוטל על ידי השואל", cancelled_by_organization: "בוטל על ידי הגמ״ח", collected: "נאסף", awaiting_return: "ממתין להחזרה", overdue: "החזרה באיחור", extension_pending: "בקשת הארכה ממתינה", returned: "הוחזר והושלם", returned_completed: "הוחזר והושלם", active: "פעיל", rejected: "נדחה", archived: "בארכיון", available: "זמין", unavailable: "לא זמין", reserved: "בתיאום" });
  ...
  function applyFilters({ resetVisible = true } = {}) {
    ...
    const query = normalize($("#search-input").value), city = $("#city-filter").value, category = state.activeCategory || $("#category-filter").value, condition = $("#condition-filter").value, availableOnly = $("#available-only").checked, type = $("#type-filter").value, pickup = $("#pickup-filter").value;
    ...
    state.filteredItems = state.items.filter(item => { const haystack = normalize([item.title, item.description, item.category, item.subcategory, ...(item.tags || []), item.city, item.neighborhood, item.organizations?.name].join(" ")); return (!query || queryTerms.some(term => haystack.includes(term))) && (!city || item.city === city) && (!category || item.category === category) && (!condition || item.condition === condition) && (!type || item.item_type === type) && (!pickup || item.pickup_method === pickup) && (!availableOnly || item.availability_status === "available"); });
    const sort = $("#sort-select").value;
    state.filteredItems.sort((a, b) => sort === "newest" ? new Date(b.created_at) - new Date(a.created_at) : sort === "city" ? String(a.city).localeCompare(String(b.city), "he") : Number(b.availability_status === "available") - Number(a.availability_status === "available") || Number(b.organizations?.rating || 0) - Number(a.organizations?.rating || 0));
    renderItems();
  }
  ...
  async function openOrganization(id) {
    ...
    try { const data = await api(`/api/organizations/${encodeURIComponent(id)}/public`); const org=data.organization, items=data.items||[], reviews=data.reviews||[], hours=Object.entries(org.hours||{});
      $("#organization-dialog-content").innerHTML = `<article class="organization-detail"><div class="organization-hero"><h2 id="organization-title">${escapeHTML(org.name)}</h2><p>${escapeHTML(org.description)}</p><div class="organization-meta"><span>📍 ${escapeHTML([org.city,org.neighborhood].filter(Boolean).join(", "))}</span><span>${org.rating ? `⭐ ${escapeHTML(org.rating)} (${Number(org.review_count||0)} ביקורות)` : "עדיין אין ביקורות"}</span><span>${org.last_active_at ? `פעיל ${escapeHTML(formatRelative(org.last_active_at))}` : "פעילות טרם עודכנה"}</span></div></div><div class="organization-facts"><section><h3>כתובת וניווט</h3><p>${escapeHTML(org.address||"לא הוזנה כתובת")}</p>${org.address?`<a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(org.address)}" target="_blank" rel="noopener">פתיחה במפה</a>`:""}</section><section><h3>טלפון</h3><p>${org.contact_phone?`<a href="tel:${escapeHTML(org.contact_phone)}">${escapeHTML(org.contact_phone)}</a>`:"לא הוזן טלפון"}</p></section><section><h3>אזור שירות</h3><p>${escapeHTML(org.service_area || org.city)}</p></section><section><h3>שעות פעילות</h3><p>${hours.length ? hours.map(([d,h]) => `${escapeHTML(d)}: ${escapeHTML(h)}`).join(" · ") : "בתיאום מראש"}</p></section></div><h3>פריטים בגמ״ח</h3><div class="mini-items">${items.map(item => `<button type="button" data-org-item="${escapeHTML(item.id)}"><strong>${escapeHTML(item.title)}</strong><span>${escapeHTML(item.availability_status === "available" ? "זמין" : "בתיאום")}</span></button>`).join("") || "אין כרגע פריטים פעילים"}</div><h3>ביקורות</h3><div class="review-list">${reviews.map(review => `<blockquote><strong>${"★".repeat(Number(review.rating))}</strong><p>${escapeHTML(review.comment || "ללא הערה")}</p><cite>${escapeHTML(review.author_name)}</cite></blockquote>`).join("") || "עדיין אין ביקורות"}</div></article>`;
  ...
  function setAuthMode(mode) {
    ...
    const register = state.authMode === "register"; $("#auth-name-field").hidden = !register; $("#auth-profile-fields").hidden=!register; ["#auth-name","#auth-phone","#auth-city","#auth-address"].forEach(selector=>$(selector).required=register); $("#auth-consent-row").hidden = !register; $("#auth-operational-row").hidden=!register; $("#auth-community-row").hidden=!register; $("#auth-consent").required = register; $("#auth-operational").required=register; $("#forgot-password-button").hidden = register; $("#auth-password").autocomplete = register ? "new-password" : "current-password"; $("#auth-title").textContent = register ? "מצטרפים לגמ״ח ברגע" : "כניסה לגמ״ח ברגע"; $("#auth-description").textContent = register ? "יוצרים חשבון בחינם ומתחילים להשאיל ולעזור." : "נכנסים עם אימייל וסיסמה כדי לבקש, לשמור ולנהל פריטים."; $("#auth-submit").textContent = register ? "יצירת חשבון" : "כניסה";
  }
  ...
  function openGmachForm(organizationId = null) {
    requireAuth(() => {
      ...
      $("#gmach-form-description").textContent = organization ? "אפשר לעדכן את פרטי הגמ״ח בכל עת." : "ממלאים פרטים בסיסיים. העמוד יפורסם אוטומטית לאחר הוספת המוצר הראשון.";
      $("#gmach-submit").textContent = organization ? "שמירת שינויים" : "יצירת הגמ״ח";
      if (organization) {
  ...
  async function openItemForm(itemId = null) {
    requireAuth(async () => { try {
      ...
      if (item) {
        ...
        $("#item-quantity").value = item.quantity || 1; $("#item-description").value = item.description || ""; $("#item-conditions").value = item.loan_conditions || ""; $("#item-type").value=item.item_type||"loan"; $("#item-pickup").value=item.pickup_method||"pickup"; $("#item-subcategory").value=item.subcategory||""; $("#item-tags").value=(item.tags||[]).join(", "); $("#item-deposit").value=item.deposit_amount??""; $("#item-max-per-user").value=item.max_per_user??""; $("#item-preparation").value=item.preparation_minutes||0; $("#item-max-days").value=item.max_loan_days??""; $("#item-radius").value=item.service_radius_km??""; $("#item-publish-at").value=item.publish_at?String(item.publish_at).slice(0,16):""; $("#item-free").checked = true;
  ...
  async function showDashboard(tab = state.dashboardTab) {
    ...
    try { const data = await refreshAccountSnapshot(); $("#stat-requests").textContent = data.stats.activeRequests; $("#stat-items").textContent = data.stats.items; $("#stat-completed").textContent = data.stats.completed; if (tab === "requests") renderDashboardRequests(data.requests || []); if (tab === "items") renderDashboardItems(data.items || []); if (tab === "gmachim") renderDashboardOrganizations(data.organizations || []); if (tab === "settings") await renderAccountSettings(); if (tab === "admin") await renderAdmin(); }
  ...
  async function renderAccountSettings() {
    const [profileData, preferencesData, ticketsData, sessionsData] = await Promise.all([api("/api/me/profile"), api("/api/me/notification-preferences"), api("/api/support/tickets"), api("/api/me/security/sessions")]);
    const profile = profileData.profile || {}, preferences = new Map((preferencesData.preferences || []).map(row => [row.notification_type, row]));
    const preferenceRow = (type, label, important = false) => { const row = preferences.get(type) || {}; return `<tr><th scope="row">${escapeHTML(label)}</th><td><input type="checkbox" data-pref-in-app="${type}" ${row.in_app !== 0 ? "checked" : ""} aria-label="באתר"></td><td><input type="checkbox" data-pref-email="${type}" ${(row.email || important) ? "checked" : ""} aria-label="אימייל"></td><td><input type="checkbox" data-pref-push="${type}" ${row.push ? "checked" : ""} aria-label="מכשיר"></td></tr>`; };
    $("#dashboard-content").innerHTML = `<section class="dashboard-subsection"><h2>פרטים אישיים</h2><p>הכתובת הפרטית מוצפנת ואינה מוצגת לציבור.</p><form id="profile-settings-form" class="stack-form"><div class="two-columns"><label><span>שם מלא</span><input id="profile-full-name" value="${escapeHTML(profile.fullName)}" required></label><label><span>טלפון</span><input id="profile-phone" dir="ltr" value="${escapeHTML(profile.phone)}" required></label></div><div class="two-columns"><label><span>עיר או יישוב</span><input id="profile-city" list="israel-cities" value="${escapeHTML(profile.city)}" required></label><label><span>שפה</span><select id="profile-language"><option value="he" ${profile.preferredLanguage !== "en" ? "selected" : ""}>עברית</option><option value="en" ${profile.preferredLanguage === "en" ? "selected" : ""}>English</option></select></label></div><label><span>כתובת מלאה</span><input id="profile-address" value="${escapeHTML(profile.address)}" required></label><label class="consent-row"><input id="profile-community-emails" type="checkbox" ${profile.communityEmailsAccepted ? "checked" : ""}><span>קבלת הודעות קהילה והתנדבות</span></label><button class="button button-primary" type="submit">שמירת הפרטים</button></form></section><section class="dashboard-subsection"><h2>התראות</h2><form id="notification-settings-form"><div class="table-wrap"><table><thead><tr><th>סוג</th><th>באתר</th><th>אימייל</th><th>מכשיר</th></tr></thead><tbody>${preferenceRow("loan_status","עדכוני השאלה",true)}${preferenceRow("availability","פריט שהתפנה",true)}${preferenceRow("community","קהילה והתנדבות")}${preferenceRow("security","אבטחה",true)}</tbody></table></div><div class="two-columns"><label><span>שעות שקטות — התחלה</span><input id="quiet-start" type="time"></label><label><span>סיום</span><input id="quiet-end" type="time"></label></div><button class="button button-primary" type="submit">שמירת ההתראות</button></form></section><section class="dashboard-subsection"><h2>אבטחה ותמיכה</h2><p>${sessionsData.sessions?.length || 0} חיבורים פעילים לחשבון.</p><button class="button button-secondary" id="logout-all-sessions" type="button">ניתוק מכל המכשירים</button><h3>פניות תמיכה</h3>${(ticketsData.tickets || []).map(ticket => `<article class="dashboard-row"><div><strong>פנייה #${ticket.ticket_number}</strong><p>${escapeHTML(ticket.subject)}</p></div><span class="status-chip">${escapeHTML(ticket.status)}</span></article>`).join("") || "<p>אין פניות קודמות.</p>"}</section>`;
    $("#profile-settings-form").addEventListener("submit", saveProfileSettings); $("#notification-settings-form").addEventListener("submit", saveNotificationSettings); $("#logout-all-sessions").addEventListener("click", revokeAllSessions);
  }
  async function saveProfileSettings(event) { event.preventDefault(); try { await api("/api/me/profile", { method:"PATCH", body:{ fullName:$("#profile-full-name").value, phone:$("#profile-phone").value, city:$("#profile-city").value, address:$("#profile-address").value, preferredLanguage:$("#profile-language").value, communityEmailsAccepted:$("#profile-community-emails").checked } }); toast("הפרטים נשמרו"); } catch(error) { toast(error.message,"error"); } }
  async function saveNotificationSettings(event) { event.preventDefault(); const types=["loan_status","availability","community","security"], quietStart=$("#quiet-start").value||null, quietEnd=$("#quiet-end").value||null; const preferences=types.map(type=>({type,inApp:document.querySelector(`[data-pref-in-app="${type}"]`).checked,email:document.querySelector(`[data-pref-email="${type}"]`).checked,push:document.querySelector(`[data-pref-push="${type}"]`).checked,quietStart,quietEnd})); try { await api("/api/me/notification-preferences",{method:"PUT",body:{preferences}}); toast("העדפות ההתראות נשמרו"); } catch(error) { toast(error.message,"error"); } }
  async function revokeAllSessions() { if(!window.confirm("לנתק את החשבון מכל המכשירים, כולל המכשיר הזה?")) return; try { await api("/api/me/security/sessions",{method:"DELETE"}); state.user=null; state.dashboard=null; updateAuthUI(); showHome(); toast("כל החיבורים נותקו"); } catch(error) { toast(error.message,"error"); } }
  ...
  async function loadSiteSettings() {
    ...
    try {
      ...
      const siteName=s.site_name||"גמ״ח ברגע", tagline=s.tagline||"גדולה גמילות חסדים יותר מן הצדקה", logo=!s.logo_url||s.logo_url==="/gmach-berega-mark.jpg"||s.logo_url==="/gmach-berega-logo.jpg"||s.logo_url==="/gmach-berega-mark-cropped.jpg"?"/gmach-berega-full.jpg":s.logo_url; $$(".brand-copy strong").forEach(el => el.textContent = siteName); $$(".brand-copy small").forEach(el => el.textContent = tagline); $("#hero-title").textContent = s.hero_title||"מה תרצו לשאול היום?"; $("#hero-title").nextElementSibling.textContent = s.hero_description||"מוצאים ציוד להשאלה בחינם מגמ״חים ואנשים טובים קרוב לבית."; $$(".brand-logo-crop img").forEach(img => img.src = logo); document.title = `${siteName} — ${tagline}`;
  ...
  async function submitReview(event) { event.preventDefault(); const button=event.submitter; setButtonBusy(button,true,"מפרסמים…"); try { await api("/api/reviews",{method:"POST",body:{requestId:$("#review-request-id").value,rating:Number($("#review-rating").value),productRating:Number($("#review-product-rating").value),serviceRating:Number($("#review-service-rating").value),comment:$("#review-comment").value}}); closeDialog($("#review-dialog")); toast("תודה! הביקורת פורסמה"); await loadDiscovery(); } catch(error){toast(error.message,"error");} finally{setButtonBusy(button,false);} }
  function resetFilters() { const hadDate = Boolean($("#date-filter").value); $("#search-form").reset(); $("#category-filter").value = ""; $("#condition-filter").value = ""; $("#type-filter").value=""; $("#pickup-filter").value=""; $("#available-only").checked = true; state.activeCategory = ""; $$('[data-category]').forEach(button => button.classList.toggle("is-active", button.dataset.category === "")); if (hadDate) loadItems(); else applyFilters(); }
  ...
  function setupEvents() {
    ...
    $("#search-form").addEventListener("submit", async event => { event.preventDefault(); if ($("#date-filter").value) await loadItems(); else applyFilters(); const details={query:$("#search-input").value.trim(),city:$("#city-filter").value,category:state.activeCategory||$("#category-filter").value}; analytics(state.filteredItems.length ? "search" : "no_results",details); $("#catalog").scrollIntoView({ behavior: "smooth", block: "start" }); }); ["#city-filter", "#category-filter", "#condition-filter", "#type-filter", "#pickup-filter", "#available-only", "#sort-select"].forEach(selector => $(selector).addEventListener("change", () => applyFilters())); $("#date-filter").addEventListener("change", loadItems);
    ...
    $("#request-form").addEventListener("submit", async event => { event.preventDefault(); const button = event.submitter; setButtonBusy(button, true, "שומרים את המלאי…"); try { await submitLoanRequest({ itemId: $("#request-item-id").value, requestedFrom: $("#request-start").value, requestedUntil: $("#request-end").value, quantity:Number($("#request-quantity").value), phone: $("#request-phone").value, note: $("#request-note").value }); event.currentTarget.reset(); closeDialog($("#request-dialog")); toast("המלאי נשמר והבקשה ממתינה לאישור זמן האיסוף."); } catch (error) { toast(error.message || "לא הצלחנו לשלוח את הבקשה", "error"); } finally { setButtonBusy(button, false); } });
    $("#gmach-form").addEventListener("submit", async event => { event.preventDefault(); const button = event.submitter; const editing = state.editingOrganizationId; setButtonBusy(button, true, editing ? "שומרים…" : "יוצרים…"); try { const body = { name: $("#gmach-name").value, primaryCategory: $("#gmach-category").value, city: $("#gmach-city").value, neighborhood: $("#gmach-neighborhood").value, description: $("#gmach-description").value, phone: $("#gmach-phone").value, address:$("#gmach-address").value,serviceArea:$("#gmach-service-area").value,hours:{"שעות":$("#gmach-hours").value},pickupOptions:$$('[name="gmach-pickup"]:checked').map(input=>input.value) }; await api(editing ? `/api/organizations/${encodeURIComponent(editing)}` : "/api/organizations", { method: editing ? "PATCH" : "POST", body }); event.currentTarget.reset(); state.editingOrganizationId = null; closeDialog($("#gmach-dialog")); toast(editing ? "פרטי הגמ״ח נשמרו" : "הגמ״ח נוצר. הוסיפו מוצר ראשון כדי לפרסם אותו."); await refreshAccountSnapshot(); if (editing) showDashboard("gmachim"); } catch (error) { toast(error.message || "לא הצלחנו לשמור את הגמ״ח", "error"); } finally { setButtonBusy(button, false); } });
    $("#item-form").addEventListener("submit", async event => { event.preventDefault(); const button = event.submitter; const editing = state.editingItemId; setButtonBusy(button, true, editing ? "שומרים…" : "מפרסמים…"); try { const body = { organizationId: $("#item-gmach").value, title: $("#item-name").value, category: $("#item-category").value, condition: $("#item-condition").value, quantity: Number($("#item-quantity").value), description: $("#item-description").value, loanConditions: $("#item-conditions").value,itemType:$("#item-type").value,pickupMethod:$("#item-pickup").value,subcategory:$("#item-subcategory").value,tags:$("#item-tags").value.split(","),depositAmount:$("#item-deposit").value,maxPerUser:$("#item-max-per-user").value,preparationMinutes:$("#item-preparation").value,maxLoanDays:$("#item-max-days").value,serviceRadiusKm:$("#item-radius").value,publishAt:$("#item-publish-at").value||null }; const data = await api(editing ? `/api/items/${encodeURIComponent(editing)}` : "/api/items", { method: editing ? "PATCH" : "POST", body }); const itemId = editing || data.item.id; await uploadItemImages(itemId, [...$("#item-images").files]); event.currentTarget.reset(); state.editingItemId = null; $("#item-gmach").disabled = false; closeDialog($("#item-form-dialog")); toast(editing ? "השינויים נשמרו" : "הפריט פורסם והגמ״ח מופיע כעת בחיפוש"); await refreshAccountSnapshot(); if (editing) showDashboard("items"); } catch (error) { toast(error.message || "לא הצלחנו לשמור את הפריט", "error"); } finally { setButtonBusy(button, false); } });
