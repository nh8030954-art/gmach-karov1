(() => {
  ...
  async function init() {
    setupEvents(); setAuthMode("login"); updateAuthUI(); await detectServer(); await Promise.all([loadSiteSettings(), loadPageCustomizations(), loadPublicConfig()]); document.documentElement.classList.remove("app-booting"); await refreshUser(); await loadItems(); registerWebMCP();
