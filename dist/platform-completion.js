(()=>{
  ...
  async function renderSearches(){
    ...
    for(const s of data.searches){const d=document.createElement("div");d.className="pt-card";d.innerHTML=`<strong>${esc(s.name)}</strong><div class="pt-muted">${esc(JSON.stringify(s.filters||{}))}</div><button class="pt-btn" type="button" data-edit>עריכה</button> <button class="pt-btn danger" type="button" data-del>מחיקה</button>`;
      d.querySelector("[data-edit]").onclick=()=>{editingId=s.id;for(const key of ["name","q","city","category"])form.elements.namedItem(key).value=key==="name"?s.name:s.filters?.[key]||"";form.elements.namedItem("notify").checked=Boolean(s.notify);form.querySelector('[type="submit"]').textContent="שמירת שינויים";form.scrollIntoView({block:"nearest"});form.elements.namedItem("name").focus()};
