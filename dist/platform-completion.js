(()=>{
  ...
  async function renderAddresses(){
    ...
    for(const x of data.addresses){const d=document.createElement("div");d.className="pt-card";d.innerHTML=`<strong>${esc(x.label)}</strong> · ${esc(x.city)} ${x.isDefault?"· ברירת מחדל":""}<div class="pt-muted">${esc(x.address||"")}</div><div class="pt-row"><button class="pt-btn" data-edit>עריכה</button><button class="pt-btn danger" data-del>מחיקה</button></div>`;d.querySelector("[data-edit]").onclick=()=>{const form=$("#pt-address-form",body);form.dataset.editId=x.id;form.label.value=x.label||"";form.city.value=x.city||"";form.address.value=x.address||"";form.isDefault.checked=Boolean(x.isDefault);form.querySelector("button").textContent="שמירת שינוי";form.scrollIntoView({behavior:"smooth",block:"center"})};d.querySelector("[data-del]").onclick=async()=>{await api("/api/me/addresses/"+encodeURIComponent(x.id),{method:"DELETE"});renderAddresses()};list.appendChild(d);}
    $("#pt-address-form",body).onsubmit=async e=>{e.preventDefault();const f=e.currentTarget;try{const id=f.dataset.editId;await api(id?"/api/me/addresses/"+encodeURIComponent(id):"/api/me/addresses",{method:id?"PATCH":"POST",body:{label:f.label.value,city:f.city.value,address:f.address.value,isDefault:f.isDefault.checked}});await renderAddresses();}catch(err){setStatus(err.message,true)}};
  }
